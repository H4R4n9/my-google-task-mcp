...
├── screenshot-agent.png       ② 자연어로 시킨 화면
│                                 에이전트가 도구를 골라 호출한 대화
└── screenshot-tasks.png       ③ 구글 할일 앱 화면
                                  실제로 만들어진 결과
```

캡처 두 장이 짝을 이룹니다. **②는 에이전트가 스스로 도구를 골랐다는 증거**이고, **③은 그게 진짜 내 계정에 반영됐다는 증거**입니다. 폴더 구조나 파일 이름은 이것만 맞으면 나머지는 자유입니다.
