...
├── RESULT.md                실습 기록. 아래 양식을 복사해서 채운다
└── screenshot.png           할일 앱에 실제로 생긴 화면 캡처
```

폴더 구조나 파일 이름은 이것만 맞으면 나머지는 자유입니다.

### RESULT.md — 이 양식을 복사해서 채우세요

대부분 **터미널이나 화면에 나온 것을 그대로 붙여넣는 것**입니다. 따로 글을 쓰지 않아도 됩니다.

````markdown
# Google Tasks MCP 실습 결과

## 1. 쓴 에이전트
<!-- Claude Code / Codex / Gemini CLI / 기타 중 하나 -->

## 2. 만든 도구 4개의 설명
<!-- 각 도구의 description 에 내가 실제로 뭐라고 썼는지 옮겨 적기 -->
...
## 3. 에이전트에게 보이는 내 도구 목록
<!-- 에이전트에게 "너가 쓸 수 있는 도구 목록을 보여줘" 라고 물어본 결과를 붙여넣기 -->

```
```

## 4. 일부러 틀린 값을 넣었을 때
<!-- 예: 마감일에 "내일" 을 넣어 호출하고, 응답 원문을 그대로 붙여넣기 -->
...
## 6. 하위 할일 검증   ← 가장 중요
<!-- add_task 에 parent_task_id 를 넣어 하위 할일을 만든 다음,
     list_tasks 로 "다시 조회한" 응답을 붙여넣는다 -->
...
의도한 대로 하위 할일이 맞았나요?
아니었다면 무엇이 문제였고 어떻게 고쳤나요?

## 7. 도구 설명을 고쳤더니
<!-- 코드는 그대로 두고 description 문구만 바꿔서 같은 질문을 다시 해보기 -->

고치기 전 설명 / 그때 에이전트가 한 행동:

고친 후 설명 / 그때 에이전트가 한 행동:
````
